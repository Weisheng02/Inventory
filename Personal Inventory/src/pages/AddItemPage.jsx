...
export default function AddItemPage() {
  ...
  if (!user) {
    return <p className="text-sm text-gray-600">Please login to add items.</p>
  }
    ...
    <motion.form onSubmit={onSubmit} className="max-w-2xl space-y-4" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <Field label="ID" error={errors.id}>
          <input name="id" value={form.id} onChange={onChange} className="w-full rounded-xl border-gray-300" />
        </Field>
